    ...
    for comment in root.findall('comment'):
        try:
            if(comment.find('commentaire').text != '' and comment.find('commentaire').text != None):
                ...
                commentClear = commentClear.replace("'", ' ')
            ...
            print("Une ligne en moins:", comment.find('commentaire').text)

    return review_lines


def convertLinesToWord2VecFormat(lines, name):
    print("converts")
    model = gensim.models.Word2Vec(sentences=lines, size=20, window=5, workers=4, min_count=1)
    words = list(model.wv.vocab)
    print('Vocabulary size: %d' % len(words))
    model.save("Models/word2vec_" + name + ".model")


root = loadFile(sys.argv[1])
lines = read(root)
convertLinesToWord2VecFormat(lines, sys.argv[1])
