...
def prepareData(filename, coef):
    listReview = []
    if Path('PreparedData/preparedData_'+filename+'.pickle').is_file():
        f = open('PreparedData/preparedData_'+filename+'.pickle', 'rb')
        (listReview,listNote) = pickle.load(f)
        f.close()
    else: 
        f = open('PreparedData/preparedData_'+filename+'.pickle', 'wb')
...
def read(root):
    ...
    for comment in root.findall('comment'):
        ...
        if(commentReview != '' and commentReview != ' ' and commentReview != None):
            ...
            commentReview = commentReview.replace("'", ' ')
...
model = Sequential()
model.add(Dense(512, input_shape=(22,), activation='relu'))
...
checkpointAccuracy = ModelCheckpoint("./bestAccuracy.h5",monitor="val_categorical_accuracy",verbose=0,save_best_only=True,save_weights_only=False)
